/* Phone mockup with cycling screens — the hero centerpiece */
const { useEffect: usePhEffect, useState: usePhState, useRef: usePhRef } = React;
...
/* Reusable status bar (top of every phone screen) */
function StatusBar({ time = "06:14" }) {
  return (
    <div className="flex justify-between items-center text-white/90 text-[11px] font-mono font-semibold tracking-wide px-1.5">
      ...
      <span className="inline-flex items-center gap-1.5">
        <span className="material-symbols-outlined ms-fill text-[14px]">signal_cellular_alt</span>
        <span className="material-symbols-outlined ms-fill text-[14px]">wifi</span>
        <span className="material-symbols-outlined ms-fill text-[14px]">battery_full</span>
      </span>
    </div>
  );
}
      ...
      <div className="flex justify-between items-center mt-1">
        <div className="flex items-center gap-2">
          ...
          <div className="leading-tight">
            <p className="text-[8px] font-mono uppercase tracking-widest text-slate-500">Operador</p>
...
function MiniPillarCard({ color, icon, label, pct }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <Icon name={icon} className="text-[16px]" style={{ color: t.hex }} />
        <span className="text-[8px] font-mono tabular" style={{ color: t.hex }}>
          {pct}%
        </span>
      </div>
...
/* Screen 2 — Cuerpo / Training session */
    ...
    <div className="absolute inset-0 bg-[#02030A] p-4 pt-3 flex flex-col gap-3 screen-in">
      ...
      <div className="flex justify-between items-center mt-1">
        ...
        <span
          ...
          style={{ background: PILLAR_TINTS.body.soft, color: PILLAR_TINTS.body.hex, border: `1px solid ${PILLAR_TINTS.body.border}` }}
        >
          CUERPO
        </span>
      </div>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#FF6B1A]/12 to-transparent border border-[#FF6B1A]/25 p-3.5 relative overflow-hidden">
        ...
        <h4 className="text-[16px] font-headline font-black-x text-white leading-tight mt-1">
...
function SetRow({ n, reps, load, status }) {
  const dot =
    status === "done"
      ? "bg-[#FF6B1A]"
      : status === "active"
      ? "bg-white ring-2 ring-[#FF6B1A]/40"
      : "bg-white/10";
...
/* Screen 3 — Combustible / Food scan */
function ScreenFuel() {
  return (
    <div className="absolute inset-0 bg-[#02030A] p-4 pt-3 flex flex-col gap-3 screen-in">
      <StatusBar />
      <div className="flex justify-between items-center mt-1">
        ...
        <span
          className="text-[9px] font-mono px-1.5 py-0.5 rounded-full"
          style={{ background: PILLAR_TINTS.fuel.soft, color: PILLAR_TINTS.fuel.hex, border: `1px solid ${PILLAR_TINTS.fuel.border}` }}
        >
          COMBUSTIBLE
        </span>
      </div>

      {/* Camera viewport with crosshair */}
...
const SCREENS = [
  { id: "dashboard", label: "Dashboard", Comp: ScreenDashboard },
  { id: "training", label: "Cuerpo · Press", Comp: ScreenTraining },
  { id: "fuel", label: "Combustible · Scan", Comp: ScreenFuel },
  { id: "complete", label: "Misión ejecutada", Comp: ScreenComplete },
];

/* Main phone shell with auto-cycle */
function PhoneStack({ tilt = 6, autoplay = true, intervalMs = 4500 }) {
  const [idx, setIdx] = usePhState(0);
  const pausedRef = usePhRef(false);
  usePhEffect(() => {
    if (!autoplay) return;
    let alive = true;
    const t = setInterval(() => {
  ...
  return (
    <div className="relative inline-block">
      ...
      {/* Foreground phone */}
      <div
        className="relative phone-shell w-[280px] h-[580px] sm:w-[300px] sm:h-[620px] lg:w-[320px] lg:h-[660px] overflow-hidden p-1.5 float-y"
      ...
      <div
        ...
        style={{ transform: `rotate(${tilt * 0.0}deg)` }}
      >
        {SCREENS.map((s, i) => (
          <button
            key={s.id}
...
/* Small static phone preview for pillar cards */
function PillarMiniPhone({ pillar }) {
  const map = {
    body: {
      hex: "#FF6B1A",
      title: "Empujón A",
  ...
  return (
    <div className="relative w-full aspect-[3/4] rounded-2xl overflow-hidden bg-[#02030A] border border-white/[0.06]">
      ...
      <div className="relative p-4 flex flex-col gap-3 h-full">
        ...
        <div>
          <p className="text-[8px] font-mono uppercase tracking-widest" style={{ color: m.hex }}>
            {m.meta}
          </p>
          <h4 className="text-[16px] font-headline font-black-x text-white tracking-tight leading-tight mt-1">
            {m.h}
          </h4>
        </div>
...
Object.assign(window, { PhoneStack, PillarMiniPhone, PILLAR_TINTS });
