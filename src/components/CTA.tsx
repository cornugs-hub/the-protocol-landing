...
export function PrimaryCTA({ href, to, onClick, children, icon, className = "" }: CTAProps) {
  const inner = (
    <>
...
export function GhostCTA({ href, to, onClick, children, icon, external = false, className = "" }: CTAProps) {
  const inner = (
    <>
      {icon ? <span className="shrink-0">{icon}</span> : null}
      <span>{children}</span>
...
export function AndroidIcon({ size = 22 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
